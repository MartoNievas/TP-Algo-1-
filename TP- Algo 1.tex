...
\subsection*{Demostración de que el valor devuelto es mayor a 50,000 utilizando la triple de Hoare}

Queremos demostrar la siguiente triple de Hoare:
\[
\{ \existe[unalinea]{i}{\ent}{0 \leq i < \longitud{ciudades} \implicaLuego ciudades[i].habitantes > 50000} \} \ S \ \{ \texttt{res} > 50,000 \}
\]
Donde el código \(S\) es el siguiente:
\begin{lstlisting}[caption={},label=code:for]
	res := 0;
	i := 0;
	while (i < ciudades.lenght()) do
...
\begin{itemize}
	...
	\[
	\existe[unalinea]{i}{\ent}{0 \leq i < \longitud{ciudades} \implicaLuego ciudades[i]}
	\]
	Sabemos que existe al menos una ciudad cuya cantidad de habitantes supera los 50,000.
	
	\item \textbf{Ciclo} (\(S\)): En cada iteración del ciclo, se va acumulando el número total de habitantes en la variable \texttt{res}. Cuando el ciclo termina, tenemos:
	\[
	\texttt{res} = \sum_{j=0}^{\texttt{ciudades.length}-1} \texttt{ciudades}[j].\texttt{habitantes}
	\]
	
	\item \textbf{Poscondición} (\(Q\)): Dado que al menos una ciudad tiene más de 50,000 habitantes y estamos sumando los habitantes de todas las ciudades, se cumple que:
	\[
	\texttt{res} > 50,000
	\]
\end{itemize}

Por lo tanto, la triple de Hoare se verifica:
\[
\{ \existe[unalinea]{i}{\ent}{0 \leq i < \longitud{ciudades} \implicaLuego ciudades[i].habitantes > 50000} \} \ S \ \{ \texttt{res} > 50,000 \}
\]
