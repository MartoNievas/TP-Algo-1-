...
\begin{quote}
	\pred{todosPositivos}{ s : \TLista{\ent}}{\paraTodo[unalinea]{i}{\ent}{0 \leq i < |s| \implicaLuego s[i] \leq 0}}
	\vspace{0.2cm}
	\pred{diagonalEnCeros}{ s : \matriz{\ent}}{\paraTodo[unalinea]{i, j}{\ent}{0 \leq i < |s| \land 0 \leq j < s[i] \land i = j \implicaLuego s[i][j] = 0}}
	\vspace{0.2cm}
	\pred{esMatrizSimetrica}{ s : \matriz{\ent}}{\paraTodo[unalinea]{i, j}{\ent}{0 \leq i < |s| \land 0 \leq j < |s[i]| \implicaLuego s[i][j] = s[j][i] \land todosPositivos(s[i])}}
	\vspace{0.2cm}
	\pred{esMatrizCuadrada}{ s : \matriz{\ent}}{\paraTodo[unalinea]{i}{\ent}{0 \leq i < |s| \implicaLuego |s[i]| = |s|}}
	\vspace*{0.2cm}
	\pred{esCamino}{ distancias : \matriz{\ent}, c : \TLista{\ent}, d : \ent, h : \ent}{(esMatrizCuadrada(distancias) \land |c| \geq 1) \yLuego (|c| \geq 1)  \paraTodo[unalinea]{e}{\ent}{e \in c \implicaLuego 0 \leq c < |distancias| \land (c[0] = d \land c[|c|-1] = h) \land \paraTodo[unalinea]{i}{\ent}{ 0 \leq i < |c|-1 \implicaLuego distancias[c[i]][c[i+1]] > 0 }}}
\end{quote}
...
\begin{quote}
	...
	\vspace{0.2cm}
	\\ 
	\pred{mimasCiudades}{ s: \TLista{Ciudad},  l: \TLista{Ciudad}}{\paraTodo[unalinea]{i}{\ent}{0 \leq i < |s| \implicaLuego \existe[unalinea]{j}{\ent}{0 \leq j < |l| \yLuego s[i].nombre = l[j].nombre}} }
	\vspace{0.2cm}
	\pred{esLaSuma}{ res : \TLista{Ciudad},  s: \TLista{Ciudad},  l: \TLista{Ciudad}}{\paraTodo[unalinea]{i}{\ent}{0 \leq i < |res| \implicaLuego \existe[unalinea]{j,k}{\ent}{0 \leq j < |s| \land 0 \leq k < |l| \yLuego res[i].habitantes = s[j].habitantes + l[k].habitantes }}}
	\vspace{0.2cm}
	...
	\begin{proc}{hayCamino}{\In distancias : \matriz{\ent}, \In desde : \ent, \In hasta : \ent}{\bool}
		\requiere{esMatrizCuadrada(distancias) \land diagonalEnCeros(distancias) \land esMatrizSimetrica(distancias) \land ( 0 \leq desde < |distancias| \land 0 \leq hasta < |distancias|)}
	...
	\begin{proc}{caminoMinimo}{\In origen : \ent, \In destino : \ent, \In distancias : \matriz{\ent}}{\TLista{\ent}}
		\requiere{(diagonalEnCeros(distancias) \land esMatrizCuadrada(distancias) \land esMatrizSimetrica(distancias) \land (0 \leq origen < \longitud{distancias} \land 0 \leq destino < \longitud{distancias}))}
		\asegura{(esCamino(res) \yLuego \paraTodo[unalinea]{c}{\TLista{\ent}}{esCamino(c) \implicaLuego distanciaRecorrida(res) \geq distanciaRecorrida(c)}) \vee (res = \lvacia) \longleftrightarrow \neg \existe[unalinea]{c}{\TLista{\ent}}{esCamino(c)}}
	\end{proc}
