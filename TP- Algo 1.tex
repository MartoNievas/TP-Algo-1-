...
\subsection{Terminacion del ciclo (mediante teroema de la terminacion).}
 ...
 Llamamo \Circled{1} a $Wp(i := i + 1, \longitud{ciudades}-i < v_0))$ y \Circled{2} a $Wp(res := res + ciudades[i].habitantes, \Circled{1}$ \\ [0.2cm]
 ...
 Ahora remplazamos el valor calculado de \Circled{1} en \Circled{2} y queda $Wp(res := res + ciudades[i].habitantes, \longitud{ciudades} - i - 1 < v_0)$ \\ [0.1cm]
 \textbf{Calculamos \Circled{2}} \\ [0.2cm]
